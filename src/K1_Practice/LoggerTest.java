...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        LogProcessor<T> firstLogProcessor = logs -> (ArrayList<T>) logs.stream().filter(iLog -> iLog.getType().equals("INFO")).collect(Collectors.toList());

        LogProcessor<T> secondLogProcessor = logs -> (ArrayList<T>) logs.stream().filter(iLog -> iLog.getMessage().length() < 100).collect(Collectors.toList());

        LogProcessor<T> thirdLogProcessor = logs -> (ArrayList<T>) logs.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toList());
